...
class SessionController{
    async create(request: Request, response: Response){
        ...
        if(!user) return response.status(400).json({error: "Usuário não informado"})
        if(!name) return response.status(400).json({error: "Nome não informado"})
        if(!module) return response.status(400).json({error: "Módulo não informado"})
        if(!content) return response.status(400).json({error: "Conteúdo não informado"})
        if(!thumbnail) return response.status(400).json({error: "Imagem não informado"})


        const moduleRepository = getCustomRepository(ModuleRepository);
        const existModule = await moduleRepository.findOne({id: module}, {relations: ['producer']});
        if(!existModule) return response.status(400).json({message: "O módulo não existe"});

        if(existModule.producer.id != user.id) return response.status(400).json({message: "Não autorizado"});

        const sessionRepository = getCustomRepository(SessionRepository);
        const existSession = await sessionRepository.findOne({name});
        if(existSession) return response.status(400).json({message: "A sessão já existe"});
    ...
    async get(request: Request, response: Response){
        ...
        if(!sessionId) return response.status(400).json({error: "Sessão não informada"})
    ...
    async createProgress(request: Request, response: Response){
        ...
        if(!session) return response.status(400).json({error: "Sessão não informada"})
    ...
    async update(request: Request, response: Response){
        ...
        const sessionRepository = getCustomRepository(SessionRepository)
        const existSession = await sessionRepository.findOne(sessionId, {relations: ['module', 'module.producer']})
        if(!existSession) return response.status(400).json({error: "Sessão não existe"})

        if(existSession.module.producer.id != user.id) return response.status(400).json({message: "Não autorizado"})

        if(thumbnail){
            const pictureRepository = getCustomRepository(PictureRepository);
            if(existSession.thumbnail) await pictureRepository.removePicture(existSession.thumbnail.id)
